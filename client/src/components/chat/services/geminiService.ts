...
function getAiInstance() {
  ...
  // Try to get from env (vite injects these at build time if prefixed with VITE_)
  // If it's missing (e.g. env not loaded properly in client context), fallback to the known key
  const apiKey = import.meta.env.VITE_GEMINI_API_KEY || 'AIzaSyCi5jOYRyg4V_QBc-D80tMgJCwl0ivUxac';
  
  if (!apiKey) {
    throw new Error("VITE_GEMINI_API_KEY nije definisan.");
  }
...
export async function getChatResponse(messages: { role: 'user' | 'model', parts: { text: string }[] }[]) {
  ...
  // Our local history starts with a 'model' greeting, so we should skip it or ensure correct order.
  const filteredMessages = messages.filter((m, index) => {
    if (index === 0 && m.role === 'model') return false;
    return true;
  });

  if (filteredMessages.length === 0) return "Izvinite, došlo je do greške u obradi poruke.";

  const lastMessage = filteredMessages[filteredMessages.length - 1].parts[0].text;
  const history = filteredMessages.slice(0, -1);

  // We explicitly mention the URL in the prompt to ensure urlContext has something to work with
  // If it's the first user message, we prepend the site context
